...
//            Float newprice = order.getNewprice();
//            int privce =
//            if (newPrice > 800) {
//                user.setLevel("1");
////            orderService.getAddressByKey()
...
//        }
        if(newPrice >= 800){
            if (newPrice<1200) {
                user.setLevel("1");
            }else if(newPrice>=1200&&newPrice<1600){
                user.setLevel("2");
            }else{
                user.setLevel("3");
            }
    ...
    @RequestMapping("/pay")
    public Msg pay(Order order,HttpSession session) {
//        User user = (User) session.getAttribute("user");
//        Order order = (Order) session.getAttribute("order") ;
    ...
    @RequestMapping("/changeIsPay")
    public void changeIsPay(Order order){
        this.orderService.updateOrderByKey(order);
