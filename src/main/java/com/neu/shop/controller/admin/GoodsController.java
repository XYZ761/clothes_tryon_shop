...
public class GoodsController {
    ...
    private String projectPath = "D:/biyesheji/shop-master1/shop-master";//request.getSession().getServletContext().getRealPath("/");
    ...
    public String addGoods(Goods goods, MultipartFile fileToUpload, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) throws IOException {
        ...
        String uploadPath = projectPath + "/shopimage/";
        int suffix = fileToUpload.getOriginalFilename().lastIndexOf('.');
        String imageName = UUID.randomUUID().toString().replace("-", "") + fileToUpload.getOriginalFilename().substring(suffix);
        fileToUpload.transferTo(new File(uploadPath + imageName));
        goodsService.addImagePath(new ImagePath(null, goods.getGoodsid(),imageName));

        redirectAttributes.addFlashAttribute("succeseMsg","服装添加成功!");
